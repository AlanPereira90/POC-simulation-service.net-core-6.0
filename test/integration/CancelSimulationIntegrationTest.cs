...
public class CancelSimulationIntegrationTest
{
  private TestClient _testClient;
  public CancelSimulationIntegrationTest()
  {
    _testClient = new TestClient();
  }

  [Fact(DisplayName = "should return 202 ACCEPTED when simulation is cancelled")]
  public async void CancelSimulationSuccess()
  {
    ...
    var response = await _testClient.Patch($"/simulations/{simulation.Id}/cancel", headers);
  ...
  public async void CancelSimulationNotFound()
  {
    ...
    var response = await _testClient.Patch($"/simulations/{id}/cancel", headers);
